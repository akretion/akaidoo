...
Moving away from ad-hoc usage towards a structured "Session" based workflow compatible
with Agents. Take inspiration from Byterover.
...
- [x] **Session Context**
  ...
  - [x] When running `akaidoo addon -o`, output to `.akaidoo/context/current.md` by
        default if no path is given.

  - [x] Update `.akaidoo/context/summary.json` with the list of addons currently in the
        context.
...
- [ ] **Mission Briefing (Session State)**
  ...
  - [ ] **Format:** Use Markdown. Include metadata header, then the Tree in a code
        block.

  - [ ] **MCP Integration:** Expose `.akaidoo/context/session.md` as a
        **Prompt/Resource** so the Agent is "born" knowing the dependency tree.
