...
FRAMEWORK_ADDONS = (
    "base", "web", "web_editor", "web_tour", "portal", "mail",
    "digest", "bus", "auth_signup", "base_setup", "http_routing",
)
...
def version_callback_for_run(value: bool): # Must be defined for typer.run
    if value:
        ...
        try: mc_version = metadata.version("manifestoo-core")
        except metadata.PackageNotFoundError: pass
...
def akaidoo_command_entrypoint(
    ...
    odoo_cfg: Optional[Path] = typer.Option(
        None, "-c", "--odoo-cfg", exists=True, file_okay=True, dir_okay=False, readable=True, resolve_path=True, envvar="ODOO_RC", help="Expand addons path from Odoo configuration file." , show_default=False
    ),
    ...
    output_file: Optional[Path] = typer.Option(None, "--output-file", "-o", help="File path to write output to.", writable=True, file_okay=True, dir_okay=False),
    clipboard: bool = typer.Option(False, "--clipboard", "-x", help="Copy file contents to clipboard.", show_default=True),
    edit_in_editor: bool = typer.Option(False, "--edit", "-e", help="Open found files in an editor.", show_default=False),
    ...
    if not final_odoo_series:
        ...
        else: echo.warning("Could not detect Odoo series. Core filtering might not work.")
    ...
    try:
        dependent_addons, missing = list_depends_command(selection, addons_set, True, True, sorter)
    except CycleErrorExit:
        raise typer.Exit(1)
    if missing: echo.warning(f"Missing dependencies: {', '.join(sorted(missing))}")
    ...
    for addon_to_scan in target_addons:
        ...
        if include_views or only_views or include_wizards: extensions.append(".xml")
            ...
            for ext in extensions:
                ...
                for found_file in files_to_check:
                    ...
                    if not (only_models or only_views):
                        ...
                        if root_name == "." and not is_root_py_file and not (is_model_file or is_view_file or is_wizard_file):
