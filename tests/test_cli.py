...
try:
    from akaidoo.cli import akaidoo_command_entrypoint, FRAMEWORK_ADDONS
    from akaidoo.cli import pyperclip as actual_pyperclip_in_cli_module
except ImportError as e:
    print(f"DEBUG: Test setup ImportError: {e}")
    print(f"DEBUG: sys.path is: {sys.path}")
    pytest.skip("Skipping CLI tests, akaidoo.cli or akaidoo_command_entrypoint not found", allow_module_level=True)
...
test_app.command(name="run")(akaidoo_command_entrypoint) # Use a simple name like "run" or keep "akaidoo_test_cmd"
...
def dummy_addons_env(tmp_path_factory):
    ...
    (framework_addon_path / "models" / "__init__.py").write_text(f"# Trivial models init for {framework_addon_name}\n")
    ...
    try:
        ...
        if isinstance(manifest_dict, dict) and 'depends' in manifest_dict and isinstance(manifest_dict['depends'], list):
            ...
            if "'depends': [" in addon_a_manifest_content_str:
                ...
                addon_a_manifest_content_str = addon_a_manifest_content_str.rstrip("}") + f", 'depends': ['{framework_addon_name}']}}"
...
def _run_cli(args, catch_exceptions=False, expected_exit_code=None):
    ...
    result = runner.invoke(test_app, str_args, prog_name="akaidoo", catch_exceptions=catch_exceptions)
...
def _get_file_names_from_output(output_str, separator=","):
    if not output_str.strip():
        return set()
    return {Path(p.strip()).name for p in output_str.strip().split(separator) if p.strip()}
...
def test_list_files_basic_addons_path(dummy_addons_env):
    ...
    framework_addon_models_init = dummy_addons_env["framework_addon_path"] / "models" / "__init__.py" 
...
def test_list_files_only_models(dummy_addons_env):
    ...
    output_full_paths_set = {Path(p.strip()).resolve() for p in result.stdout.strip().split(',') if p} 
    addon_a_root_init_path = (dummy_addons_env["addon_a_path"] / "__init__.py").resolve()
...
def test_list_files_only_target_addon(dummy_addons_env):
    ...
    addon_a_root_init_path = str((dummy_addons_env["addon_a_path"] / "__init__.py").resolve())
    addon_a_models_init_path = str((dummy_addons_env["addon_a_path"] / "models" / "__init__.py").resolve())
...
def test_list_files_edit_mode(dummy_addons_env, mocker):
    ...
    assert any("__manifest__.py" in Path(arg).name for arg in called_cmd if isinstance(arg, str) and os.path.sep in arg)
